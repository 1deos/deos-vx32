 ...
 */
static int
netifgen(Chan *c, char *name, Dirtab *vp, int s, int i, Dir *dp)
{
	Qid q;
	Netif *nif = (Netif*)vp;
	Netfile *f;
...
static Cmdtab netcmds[] = {
	{CMconnect,		"connect",	2},
	{CMpromiscuous,	"promiscuous",	1},
	{CMscanbs,		"scanbs",	0},
	{CMbridge,		"bridge",	1},
	{CMheadersonly,	"headersonly",	1},
	{CMaddmulti,		"addmulti",	2},
	{CMremmulti,		"remmulti",	2},
	{CMwild,			"*",		0},
};
...
netifwrite(Netif *nif, Chan *c, void *a, long n)
{
	...
	ct = lookupcmd(cb, netcmds, nelem(netcmds));
	...
	case CMheadersonly:
		...
		break;
	case CMaddmulti:
		if(parseaddr(binaddr, cb->f[1], nif->alen) < 0)
			error("bad address");
	...
	case CMremmulti:
		...
		break;
	default:
		/* pass unrecognised commands back up */
		n = -1;
		break;
	}
